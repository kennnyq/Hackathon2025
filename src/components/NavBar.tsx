...
export default function NavBar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-red-100/70 bg-white/70 backdrop-blur-xl">
      <nav className="mx-auto flex max-w-6xl items-center gap-5 px-4 py-3 text-slate-900">
        <Link
          href="/"
          className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-2.5 py-1 shadow-[0_8px_20px_rgba(15,23,42,0.08)] transition hover:border-red-200"
        >
          <span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-white shadow-[0_6px_16px_rgba(15,23,42,0.12)]">
            <Image src="/toyotatinder.png" alt="ToyotaTinder" width={28} height={28} className="h-6 w-6 object-contain" priority />
          </span>
          <span className="brand-wordmark text-lg font-semibold text-slate-900">ToyotaTinder</span>
        </Link>

        <div className="ml-auto flex items-center gap-1 rounded-full border border-slate-200/70 bg-white/80 px-2 py-1 text-sm font-semibold shadow-[0_12px_35px_rgba(15,23,42,0.08)]">
          <Link className="nav-pill" href="/find">Find</Link>
          <Link className="nav-pill" href="/liked">Liked</Link>
